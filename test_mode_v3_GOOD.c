  ...
  /* for(int count1 =	QUESTIONS_NUM ; count1 > 0 ; count1--)
   ...
   else
   {
       ...
       while(questionCount < QUESTIONS_NUM)
       {
					...
					while(SensorValue[S3] == (int)colorBlack){}
           if((SensorValue[S3] == (int)colorRed  ||SensorValue[S3] == (int)colorGreen || SensorValue[S3] == (int)colorBlue || SensorValue[S3] == (int)colorYellow) && SensorValue[S3] != ((int)colorWhite	|| (int)colorBlack))
           {
